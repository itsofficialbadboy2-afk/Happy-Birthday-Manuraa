/**
 * BIRTHDAY QUEST - EASY & FUN EDITION
 */
...
function showSuccessPopup(nextLv) {
    let message = "";
    switch(nextLv) {
        case 3: message = "Well done, Manuraa! The heart is fully charged!"; break;
        case 4: message = "Great job, Manuraa! You caught every heart!"; break;
        case 5: message = "You fly so well Manuraa! Next level is waiting!"; break;
        case 6: message = "Well done, Manuraa! You’re my girl...winning is just who you are!"; break;
        default: message = "Congratulations, Manuraa! You’ve conquered this level too!";
    }
...
function initStage3() {
    ...
    function loop() {
        ...
        if(Math.random() < 0.025) fallingHearts.push({x: Math.random() * 250, y: 0});
...
// --- STAGE 4: FLAPPY HEART (Super Easy Edition) ---
function initStage4() {
    ...
    c.onpointerdown = (e) => { e.preventDefault(); velocity = -3.2; };

    function loop() {
        if(isGameOver || !document.getElementById('stage-4').classList.contains('active')) return;
        ctx.clearRect(0, 0, 280, 300);
        
        // "Floaty" gravity (0.1 is very slow)
        velocity += 0.1; 
        heartY += velocity;
        
        // Pipes appear less often
        if(frame % 200 === 0) {
            pipes.push({
                x: 280, 
                top: Math.random() * 80 + 40, 
                passed: false,
                gap: 130 // Much wider gap than before
            });
        }
        
        pipes.forEach((p, i) => {
            p.x -= 1; // Slow horizontal speed
            ctx.fillStyle = "#ffafcc"; 
            ctx.fillRect(p.x, 0, 40, p.top); 
            ctx.fillRect(p.x, p.top + p.gap, 40, 300);

            // Forgiving Collision: Checks if heart is deep inside the pipe
            let heartX = 40;
            let buffer = 8; // Small buffer means she can "scrape" the pipes without dying
            if (heartX + 20 - buffer > p.x && heartX + buffer < p.x + 40) {
                if (heartY + buffer < p.top || heartY + 20 - buffer > p.top + p.gap) {
                    isGameOver = true;
                }
            }
        ...
        // Bottom/Top bounds (only die if she really goes off screen)
        if (heartY > 310 || heartY < -10) isGameOver = true;
        ...
        if(globalFlyScore >= 5) {
            ...
            popup.innerHTML = `
                <div class="popup-content">
                    <h2 class="pixel-text">Opps! 💖</h2>
                    <p>It's okay baby... we can always try again!</p>
                    <button id="f-retry">RETRY</button>
                </div>
            `;
...
function openFinalLetter() {
    ...
    setTimeout(() => {
        ...
        let charIndex = 0;
        let message = "Hey baby happy birthday ..I pray for you to win every difficulty in life ..I know you will win everything in life... I have nothing else to gift you ... but i gift you an honesty ... I will never lie to you ... I will never leave you... I will never stop loving you ... I will be forever with you... I will love you for forever Manuraa ..in this life and many more to come ";
